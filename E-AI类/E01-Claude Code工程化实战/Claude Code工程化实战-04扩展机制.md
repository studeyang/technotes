...
如果 Claude 没有工具，它只能做一件事：输出文本。它能思考、能分析、能给建议，但不能行动。
...
Claude（模型）负责推理，Tools 负责行动。而 Claude Code 扮演的角色是  Agentic Harness，它提供执行环境、上下文管理、权限控制等基础设施。

<img src="https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605201926793.jpeg" alt="img" style="zoom: 25%;" />
...
Claude 的“眼睛”——在动手之前先看清楚代码库的全貌。Glob 按文件名模式定位文件，Grep 按内容搜索定位代码行，两者都是只读操作。

| 工具 | 功能           | 需要权限 | 典型用法               |
| ---- | -------------- | -------- | ---------------------- |
| Glob | 按模式匹配文件 | 否       | 找到所有 *.ts 文件     |
| Grep | 搜索文件内容   | 否       | 搜索函数定义、查找引用 |

- 执行类

整个工具集里能力最强也最危险的一个。Bash 可以执行操作系统能做的一切。

| 工具 | 功能            | 需要权限 | 典型用法                     |
| ---- | --------------- | -------- | ---------------------------- |
| Bash | 执行 Shell 命令 | 是       | 运行测试、Git 操作、构建项目 |
...
| 工具      | 功能         | 需要权限 | 典型用法                   |
| --------- | ------------ | -------- | -------------------------- |
| WebFetch  | 获取网页内容 | 是       | 查阅文档、获取 API 响应    |
| WebSearch | 搜索互联网   | 是       | 查找错误解决方案、技术文档 |

- 编排类

这组工具不直接操作代码，而是管理 Agent 的工作流程。

| 工具            | 功能               | 需要权限 | 典型用法                   |
| --------------- | ------------------ | -------- | -------------------------- |
| Task            | 创建子代理执行任务 | 是       | 委派复杂任务给专门的子代理 |
| AskUserQuestion | 向用户提问         | 否       | 需要用户输入时获取信息     |
| TodoWrite       | 管理任务清单       | 否       | 跟踪多步骤任务进度         |
...
| 工具         | 功能                | 需要权限 | 典型用法                 |
| ------------ | ------------------- | -------- | ------------------------ |
| TaskOutput   | 获取后台任务输出    | 否       | 检查后台运行的命令结果   |
| TaskStop     | 停止后台任务        | 否       | 终止长时间运行的命令     |
| MCPSearch    | 搜索和加载 MCP 工具 | 否       | 动态发现可用的 MCP 能力  |
| ExitPlanMode | 退出规划模式        | 是       | 从只读规划切换到执行模式 |

不同类别的工具有不同的风险等级：

<img src="https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605201926490.jpeg" alt="img" style="zoom: 33%;" />

## 从原语到涌现——工具设计哲学

看完工具清单，你可能会问：只有十几个内置工具，凭什么覆盖几乎所有软件工程任务？
...
<img src="https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605201927551.jpeg" alt="img" style="zoom:33%;" />

任何复杂任务，无论是“重构整个模块”还是“搭建 CI/CD 流水线”，都是这五种操作的不同排列组合。

![img](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605201928607.jpeg)
...
| 层次        | 原语                         | 数量      | 衍生能力         |
| ----------- | ---------------------------- | --------- | ---------------- |
| CPU 指令集  | MOV, ADD, JMP, CMP......     | ~几十条   | 一切可计算的程序 |
| Unix 工具   | cat, grep, sed, awk, pipe    | ~十几个   | 一切文本处理任务 |
| LEGO 积木   | 基础砖块、连接件             | ~几种形状 | 无限造型         |
| Claude Code | Read, Edit, Bash, Grep...... | ~十几个   | 一切软件工程任务 |

Unix 的设计哲学是“做好一件事”（Do One Thing Well），然后通过管道组合。Claude Code 的工具设计完全继承了这个思想。
...
![img](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605201928158.jpeg)
...
| 高级能力 | 工具组合                                           |
| -------- | -------------------------------------------------- |
| 代码重构 | Glob → Grep → Read → Edit → Bash (test)            |
| Bug 调试 | Grep → Read → Bash (加日志) → Read (看输出) → 循环 |
| 功能开发 | WebSearch → Read → Write → Edit → Bash(test)       |
| 代码审查 | Glob → Read → Grep → 输出报告                      |
| 性能优化 | Bash (分析) → Read → Edit → Bash (基准测试) → 对比 |

**Bash：图灵完备的万能逃逸舱**

在所有工具中，Bash 最特殊。它是唯一一个“不确定做什么”的工具（其他工具都有明确职责）
...
2. 权限精细控制：你可以允许 Read 但禁止 Bash，这样 Claude 能看代码但不能执行任意命令。如果只有 Bash，就无法精细控制权限。
3. Token 效率：Bash 输出通常包含大量冗余信息。Grep 等专用工具返回精简的、Claude 直接需要的信息。

背后的设计原则是，Bash 提供能力的完备性，专用工具提供交互的效率和可控性。二者缺一不可。
...
| 工具类型  | 示例             | 需要审批 | "不再询问"的作用范围      |
| --------- | ---------------- | -------- | ------------------------- |
| 只读工具  | Read, Glob, Grep | 否       | 不适用                    |
| Bash 命令 | Shell 执行       | 是       | 永久（按项目目录 + 命令） |
| 文件修改  | Edit, Write      | 是       | 会话内                    |
...
| 层级    | 位置                             | 共享范围          | 典型用途        |
| ------- | -------------------------------- | ----------------- | --------------- |
| Managed | `/etc/claude-code/settings.json` | 组织全体          | IT 策略强制执行 |
| Project | `.claude/settings.json`          | 团队（Git 提交）  | 项目级权限控制  |
| Local   | `.claude/settings.local.json`    | 个人（gitignore） | 个人项目偏好    |
| User    | `~/.claude/settings.json`        | 个人所有项目      | 全局偏好        |

高优先级的配置覆盖低优先级。如果项目配置禁止了某个操作，用户配置无法覆盖。

> 权限规则语法

在  settings.json 中可以精细配置工具权限，规则评估顺序：deny → ask → allow。deny 规则总是优先。
...
| 模式           | 含义                          | 示例                     |
| -------------- | ----------------------------- | ------------------------ |
| //path         | 文件系统绝对路径              | //Users/alice/secrets/** |
| ~/path         | 用户主目录相对路径            | ~/Documents/*.pdf        |
| /path          | settings 文件所在目录相对路径 | /src/**/*.ts             |
| path 或 ./path | 当前工作目录相对路径          | *.env                    |
...
| 参数                | 作用                     | 工具是否可见                 |
| ------------------- | ------------------------ | ---------------------------- |
| `--tools`           | 限制可用工具集           | 只有列出的工具可见           |
| `--allowedTools`    | 预授权工具（不弹窗）     | 所有工具可见，指定的自动批准 |
| `--disallowedTools` | 禁用工具（从上下文移除） | 指定的工具不可见也不可用     |
...
| 机制           | 控制什么              | 粒度        |
| -------------- | --------------------- | ----------- |
| CLI 参数       | 整个会话的工具可用性  | 会话级      |
| settings.json  | 持久化的权限规则      | 项目/用户级 |
| Sub-Agent 配置 | 特定子代理的工具集    | 代理级      |
| Skill 配置     | 特定技能的工具限制    | 技能级      |
| Hooks          | 工具执行时的拦截/增强 | 调用级      |

这种分层设计让你可以在不同粒度上精确控制 Claude 的能力边界。

## 总结一下

这一讲我们像庖丁解牛般，把 Claude Code 的工具系统彻底拆开来看了看。

**Agentic Loop** 是 Claude Code 的工作核心：收集上下文 → 采取行动 → 验证结果，循环往复。

工具是行动的载体，没有工具的 Claude 只能输出文本。Claude Code 作为 Agentic Harness，把模型和工具连接起来。

**工具设计哲学**遵循 Unix 原则——十几个精选的原语工具覆盖了五个原子操作：感知（Read）、搜索（Glob/Grep）、修改（Edit/Write）、执行（Bash）、获取（Web）。

涌现公式：原语工具 × LLM 推理 × 反馈循环 = 无限复杂能力。重构、调试、部署这些高级能力不需要专门的工具，它们是原语工具在 LLM 推理驱动下的涌现行为。
