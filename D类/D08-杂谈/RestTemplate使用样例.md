...
public BaseResponse example(ApplySignRequest request) {
    ...
    return restTemplateWrapper.postForObject(baseUrl + "/example",
                                             json,
                                             BaseResponse.class);
}
```
...
public BaseResponse update(UpdateSignRequest request) {
    String json = JSON.toJSONString(request);
    return restTemplateWrapper.putForObject(baseUrl + "/sms/sign",
                                            json,
                                            BaseResponse.class);
}
```
...
```java
public <T, R, B> List<R> exchangeForList(String url, 
...
public List<User> example() {
    return restTemplateWrapper.exchangeForList(baseUrl + "/user",
                                        HttpMethod.GET
                                        new HttpHeaders()
                                        null,
                                        User[].class);
