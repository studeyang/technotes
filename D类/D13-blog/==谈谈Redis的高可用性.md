...
前几天我在知乎看到一个关注的问题：如何建立自己的知识体系和观点？^[1]^
...
![查理芒格](https://technotes.oss-cn-shenzhen.aliyuncs.com/2022/202212102109377.webp)
...
>- [2] https://github.com/studeyang/technotes
...
![图源：极客时间《Redis核心技术与实战》-蒋德均](https://technotes.oss-cn-shenzhen.aliyuncs.com/2022/learn/20220212221818.png)

今天我们主要关注 Redis 的高可用主线。

我们常说 Redis 系统具有高可用性，具体来说，有两方面含义：一是服务少中断，二是数据少丢失。
...
<img src="https://technotes.oss-cn-shenzhen.aliyuncs.com/2022/learn/20220308215130.png" alt="Redis读写分离"  />
...
Redis 通过主从库模式，既提高了系统处理请求的吞吐量，也保证了系统的可用性。如果从库发生故障了，客户端可以继续向主库或其他从库发送请求，进行相关的操作。

但是如果主库发生故障了怎么办？此时从库没有相应的主库可以进行数据复制操作了，且一旦有写操作请求，系统将无法处理。

# 二、哨兵机制

所以，如果主库挂了，我们就需要运行一个新主库，比如说把一个从库切换为主库，把它当成主库。在 Redis 主从集群中，哨兵机制是实现主从库自动切换的关键机制。

![Redis哨兵机制](https://technotes.oss-cn-shenzhen.aliyuncs.com/2022/image-20221124174919856.png)
